...
function makePlatformWalkthroughPlan(params) {
    ...
    const mode = params.wantsPromptSubmission
        ? "prompt-and-monitor"
...
function makePlatformObservationReport(params) {
    ...
    const platformState = params.loginState === "logged-out"
        ? "logged-out"
    ...
    return {
        ...
        risks,
        recommendedNextAction: platformState === "logged-out"
...
function makeMcpConnectionPlan(params) {
    ...
    const status = connected && hasTools ? "connected" : connected ? "needs-tool-discovery" : authMode === "unknown" ? "needs-auth" : "not-connected";
    return {
        status,
        serverUrl,
        authMode,
        researchPreviewWarning: "Lovable MCP is documented by Lovable as a research-preview server, so tool names, parameters, and response shapes may change. ClawKit should discover tools at runtime and keep a browser/GitHub fallback.",
        connectionSteps: [
...
function makeMcpWorkflowPlan(params) {
    ...
    const preferredSurface = needsApproval
        ? "ask-user"
        : !params.mcpConnected
            ? "browser-walkthrough"
    ...
    return {
        ...
        preferredSurface,
        reason: preferredSurface === "ask-user"
...
export default definePluginEntry({
    ...
    register(api) {
        ...
        api.registerTool({
            ...
            description: "Create an approval-gated browser walkthrough plan so OpenClaw can open Lovable.dev, inspect the actual platform screens, press approved buttons, capture evidence, and understand the project state.",
            parameters: Type.Object({
                projectName: Type.Optional(Type.String()),
        ...
        api.registerTool({
            ...
            description: "Turn OpenClaw browser observations from Lovable.dev into structured project evidence, risks, next actions, and tool routing.",
            parameters: Type.Object({
                projectName: Type.Optional(Type.String()),
        ...
        api.registerTool({
            ...
            description: "Plan a safe connection to Lovable's MCP server, including OAuth/API-key setup, tool discovery, safety rules, and browser/GitHub fallbacks.",
            parameters: Type.Object({
        ...
        api.registerTool({
            ...
            description: "Choose whether to use Lovable MCP, browser walkthrough, or GitHub/code tools for creating, iterating, inspecting, deploying, or handing off a Lovable project.",
            parameters: Type.Object({
                projectName: Type.Optional(Type.String()),
