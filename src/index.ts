...
type ProjectMemory = {
  ...
  currentGoal: string;
  sourceOfTruth: {
    primary: "lovable" | "github" | "local-repo" | "deployed-app" | "unknown";
  ...
  knownBugs: string[];
  refactorDecisions: string[];
  doNotChange: string[];
  pendingLovablePrompts: string[];
  githubTasks: string[];
  visualQaNotes: string[];
  releaseReadiness: "blocked" | "needs-work" | "ready-for-verification" | "ready-to-ship";
  changedSinceLastSession: string[];
  nextMemoryUpdate: string[];
...
type DecisionLog = {
  ...
  entries: Array<{
    ...
    decision: string;
    reason: string;
    owner: "user" | "openclaw" | "lovable" | "github" | "shared";
...
type NextActionPlan = {
  projectName: string;
  recommendedAction: "ask-user" | "prompt-lovable" | "inspect-github" | "edit-code" | "verify-visible-result" | "prepare-pr" | "ship";
  reason: string;
...
type WorkflowState = {
  projectName: string;
  mode: "new-build" | "rescue" | "improve" | "harden" | "ship" | "publish" | "unknown";
  sourceOfTruth: "lovable.dev" | "github" | "local-repo" | "deployed-app" | "unknown";
  appStatus: "idea" | "generated" | "broken" | "invisible-change" | "needs-refactor" | "ready-for-verification" | "ready-for-pr" | "ready-to-publish" | "published";
  repoStatus: "none" | "repo-url-known" | "local-repo-known" | "dirty" | "clean" | "unknown";
...
type StudioBrain = {
  projectName: string;
  mode: "start" | "rescue" | "improve" | "harden" | "ship" | "publish" | "orient-user";
...
type UserOnboarding = {
  headline: string;
  shortestPath: string[];
  askOnlyThisFirst: string[];
  userCanSay: string[];
  choices: string[];
  reassuringRules: string[];
  nextPrompt: string;
};
...
type PublishReadiness = {
  projectName: string;
  readiness: "blocked" | "needs-approval" | "needs-verification" | "ready";
  score: number;
  blockers: string[];
  missingEvidence: string[];
  approvalRequired: string[];
  recommendedNextAction: string;
  requiredPrePublishChecks: string[];
};

type PublishPlan = {
  projectName: string;
  publishGoal: string;
  preferredPath: "lovable-mcp" | "browser-walkthrough" | "github-external-deploy" | "ask-user";
  reason: string;
  accessExpectation: "public" | "workspace" | "custom-domain" | "unknown";
...
type PublishProjectPlan = {
  ...
  githubDeployInstructions: string[];
  expectedResult: string[];
  stopConditions: string[];
};

type PublishResultReport = {
  projectName: string;
  status: "published" | "failed" | "unknown";
  liveUrl: string | null;
  accessLevel: "public" | "workspace" | "custom-domain" | "unknown";
  verification: string[];
  risks: string[];
  followUp: string[];
  summary: string;
...
type ProjectDashboard = {
  projectName: string;
  mode: "plan" | "start" | "rescue" | "improve" | "harden" | "ship" | "publish" | "unknown";
  sourceOfTruth: "lovable" | "github" | "local-repo" | "deployed-app" | "unknown";
  status: "idea" | "building" | "generated" | "broken" | "needs-verification" | "verified" | "ready-to-ship" | "published";
  confidence: number;
  creditRisk: "low" | "medium" | "high";
...
type PublishConfidence = {
  projectName: string;
  score: number;
  level: "low" | "medium" | "high";
  verdict: "do-not-publish" | "needs-review" | "ready-with-approval" | "ready";
  blockers: string[];
  boosts: string[];
  penalties: string[];
  nextAction: string;
...
type ClientHandoffReport = {
  projectName: string;
  audience: "client" | "internal" | "agency" | "investor" | "team";
  status: "draft" | "ready-for-review" | "published" | "blocked";
  liveUrl: string | null;
...
type PromptLint = {
  projectName: string;
  riskLevel: "low" | "medium" | "high";
  shouldSendToLovable: boolean;
  findings: string[];
  missingSections: string[];
  splitIntoPrompts: string[];
...
type EndToEndPlan = {
  projectName: string;
  outcome: string;
  mode: "new-build" | "existing-app" | "rescue-to-live" | "unknown";
  phases: Array<{
    phase: string;
    owner: "Lovable.dev" | "OpenClaw" | "GitHub" | "Browser" | "User";
    goal: string;
    tools: string[];
    exitCriteria: string[];
  }>;
  firstUserQuestions: string[];
  creditGuardrails: string[];
  approvalGates: string[];
...
function makeProjectMemory(params: {
  ...
  localRepoPath?: string;
  deployedUrl?: string;
  primarySourceOfTruth?: "lovable" | "github" | "local-repo" | "deployed-app" | "unknown";
  stack?: string[];
  importantRoutes?: string[];
  knownBugs?: string[];
  refactorDecisions?: string[];
...
function makeDecisionLog(params: {
  ...
  const entries = [
    ...
    owner: entry.owner ?? "shared" as const,
    status: entry.status ?? "accepted" as const,
  ...
  return {
    ...
    markdown: [
      ...
      ...(entries.length > 0
        ? entries.map((entry) =>
            `- ${entry.date} [${entry.status}] ${entry.decision} Owner: ${entry.owner}. Reason: ${entry.reason}${entry.followUp.length ? ` Follow-up: ${entry.followUp.join("; ")}` : ""}`,
          )
...
function makeNextActionPlan(params: {
  ...
  hasPendingLovablePrompt?: boolean;
  readyForPr?: boolean;
}): NextActionPlan {
  const projectName = params.projectName ?? params.memory?.projectName ?? "Lovable project";
  ...
  const recommendedAction: NextActionPlan["recommendedAction"] =
    !hasRepo
      ? "ask-user"
...
function estimateUserStress(message?: string, explicit?: "calm" | "focused" | "heated" | "critical"): WorkflowState["userStress"] {
  if (explicit) {
    return explicit;
  }
...
function makeWorkflowState(params: {
  ...
  attemptedLovablePrompts?: number;
  sameIssueRepeated?: boolean;
  budgetSensitivity?: "low" | "medium" | "high";
  userMessage?: string;
  userStress?: "calm" | "focused" | "heated" | "critical";
  ...
  const mode: WorkflowState["mode"] =
    params.wantsPublish || goal.includes("publish") || goal.includes("deploy") || goal.includes("live url") || goal.includes("go live")
      ? "publish"
      : params.readyForPr
      ? "ship"
        ...
        : params.needsArchitectureRefactor || goal.includes("refactor") || goal.includes("maintain")
          ...
          : params.hasExistingApp || params.hasLovableProjectUrl || hasRepo
            ...
            : params.userGoal
              ? "new-build"
              : "unknown";
  const sourceOfTruth: WorkflowState["sourceOfTruth"] =
    params.hasLocalRepo ? "local-repo" : params.hasGithubRepo ? "github" : params.hasLovableProjectUrl ? "lovable.dev" : "unknown";
  const appStatus: WorkflowState["appStatus"] =
    params.alreadyPublished
      ? "published"
      : mode === "publish"
        ? "ready-to-publish"
        : params.readyForPr
  ...
  const repoStatus: WorkflowState["repoStatus"] =
      ...
      : params.hasLocalRepo
        ...
        : params.hasGithubRepo
          ...
          : params.hasExistingApp || params.hasLovableProjectUrl
            ? "none"
            : "unknown";
  const creditRisk: WorkflowState["creditRisk"] =
    params.budgetSensitivity === "high" || repeatedPromptRisk || hasHardFailure || params.hasInvisibleChanges
      ? "high"
...
function makeStudioBrain(params: {
  ...
  const mode: StudioBrain["mode"] =
    workflowState.mode === "new-build"
      ? "start"
...
function makePublishReadiness(params: {
  ...
  previewUrl?: string;
  buildPassed?: boolean;
  visibleResultVerified?: boolean;
  hasConsoleErrors?: boolean;
  hasSecretsConfigured?: boolean;
  desiredAccess?: "public" | "workspace" | "custom-domain" | "unknown";
  hasUserApproval?: boolean;
  hasLovableMcpConnected?: boolean;
  hasDeployTool?: boolean;
  hasLoggedInBrowserSession?: boolean;
  hasGithubRepo?: boolean;
  publishTarget?: "lovable" | "external";
}): PublishReadiness {
  const projectName = params.projectName ?? "Lovable.dev project";
  ...
  const readiness: PublishReadiness["readiness"] =
    blockers.length > 0
      ? "blocked"
      : params.hasUserApproval !== true
        ? "needs-approval"
...
function makePublishResultReport(params: {
  ...
  verification?: string[];
  risks?: string[];
  publishSurface?: "lovable-mcp" | "browser-walkthrough" | "github-external-deploy" | "unknown";
}): PublishResultReport {
...
function makePublishConfidence(params: {
  ...
  hasConsoleErrors?: boolean;
  hasSecretsConfigured?: boolean;
  desiredAccessKnown?: boolean;
  hasExplicitApproval?: boolean;
  hasPublishRoute?: boolean;
  hasRollbackPlan?: boolean;
  hasLiveUrlVerification?: boolean;
}): PublishConfidence {
  const boosts: string[] = [];
  ...
  const add = (condition: boolean | undefined, points: number, boost: string, penalty: string, blocker?: string) => {
    if (condition === true) {
      score += points;
      boosts.push(boost);
    } else {
  ...
  const verdict: PublishConfidence["verdict"] =
    blockers.length > 0 || params.buildPassed === false
      ? "do-not-publish"
...
function makeProjectDashboard(params: {
  projectName?: string;
  mode?: ProjectDashboard["mode"];
  sourceOfTruth?: ProjectDashboard["sourceOfTruth"];
  status?: ProjectDashboard["status"];
  lovableProjectUrl?: string;
  previewUrl?: string;
  githubRepoUrl?: string;
  localRepoPath?: string;
...
function makeClientHandoffReport(params: {
  projectName?: string;
  audience?: ClientHandoffReport["audience"];
  status?: ClientHandoffReport["status"];
  liveUrl?: string;
  previewUrl?: string;
...
function makePromptLint(params: {
  ...
  const riskLevel: PromptLint["riskLevel"] = highRisk ? "high" : findings.length || missingSections.length > 2 ? "medium" : "low";
...
function makeVisualQaReport(params: {
  ...
  const status: VisualQaReport["status"] =
    params.buildPassed === false || missingExpectedElements.length > 0 || consoleAndNetworkRisks.length > 0
      ? "fail"
      : findings.length > 0
        ? "needs-review"
        : "pass";
...
function makeEndToEndPlan(params: {
  ...
  hasExistingApp?: boolean;
  wantsPublish?: boolean;
  wantsGithubHandoff?: boolean;
  wantsClientHandoff?: boolean;
  budgetSensitivity?: "low" | "medium" | "high";
  knownConstraints?: string[];
}): EndToEndPlan {
  const projectName = params.projectName ?? "Lovable.dev project";
  const outcome = params.outcome ?? "Take the project from idea or current state to a verified deliverable.";
  const mode: EndToEndPlan["mode"] = params.hasExistingApp ? (params.wantsPublish ? "rescue-to-live" : "existing-app") : outcome ? "new-build" : "unknown";
  const phases: EndToEndPlan["phases"] = [
    {
      phase: "Orient",
      owner: "OpenClaw",
...
export default definePluginEntry({
  ...
  register(api) {
    ...
    api.registerTool({
      ...
      description:
        "Create or refresh durable project memory with source of truth, stack, routes, bugs, decisions, do-not-change rules, pending Lovable prompts, GitHub tasks, visual QA notes, and release readiness.",
      parameters: Type.Object({
        projectName: Type.Optional(Type.String()),
    ...
    api.registerTool({
      ...
      description:
        "Create a dated decision log for product, source-of-truth, refactor, visual QA, and delivery decisions so future OpenClaw sessions know what was agreed.",
      parameters: Type.Object({
        projectName: Type.Optional(Type.String()),
    ...
    api.registerTool({
      ...
      description:
        "Summarize what ClawKit knows at the start of a session: current goal, source of truth, changes since last time, do-not-touch rules, risks, and recommended tool order.",
      parameters: Type.Object({
        projectName: Type.Optional(Type.String()),
    ...
    api.registerTool({
      ...
      description:
        "Choose the safest next action from project memory: ask the user, prompt Lovable, inspect GitHub, edit code, verify the visible result, prepare a PR, or ship.",
      parameters: Type.Object({
        projectName: Type.Optional(Type.String()),
        memory: Type.Optional(Type.Any()),
    ...
    api.registerTool({
      ...
      description:
        "Check whether a Lovable project is ready to publish or update a live URL, including verification, access, secrets, approval, and available publish surface.",
      parameters: Type.Object({
        projectName: Type.Optional(Type.String()),
    ...
    api.registerTool({
      ...
      label: "Plan Publish",
      description:
        "Create an approval-gated publishing plan for Lovable MCP, Lovable browser walkthrough, or external GitHub deployment.",
      parameters: Type.Object({
        projectName: Type.Optional(Type.String()),
    ...
    api.registerTool({
      ...
      description:
        "Prepare the explicit publish execution route after approval. The plugin does not secretly deploy; OpenClaw must use the trusted Lovable MCP, browser, or GitHub/external host surface and then report the result.",
      parameters: Type.Object({
        projectName: Type.Optional(Type.String()),
        projectIdOrUrl: Type.Optional(Type.String()),
        publishTarget: Type.Optional(Type.Union([
    ...
    api.registerTool({
      ...
      description:
        "Record the Lovable or external deployment result: live URL, access level, verification, risks, follow-up, and rollback/unpublish notes.",
      parameters: Type.Object({
        projectName: Type.Optional(Type.String()),
        status: Type.Optional(Type.Union([
    ...
    api.registerTool({
      ...
      description:
        "Create a user-friendly ClawKit dashboard card summarizing mode, source of truth, status, confidence, links, blockers, approvals, evidence, risks, and next best action.",
      parameters: Type.Object({
        projectName: Type.Optional(Type.String()),
    ...
    api.registerTool({
      ...
      description:
        "Create a publish confidence score and verdict from readiness signals so the user gets a clear go/no-go before making a Lovable project live.",
      parameters: Type.Object({
        projectName: Type.Optional(Type.String()),
    ...
    api.registerTool({
      ...
      description:
        "Generate a client-ready handoff report with live/preview/repo links, what was built, verification, access notes, limitations, next steps, and maintenance plan.",
      parameters: Type.Object({
        projectName: Type.Optional(Type.String()),
    ...
    api.registerTool({
      ...
      description:
        "Review a proposed Lovable prompt for credit-waste risk, missing preserve/change/avoid/acceptance sections, over-broad scope, and work OpenClaw should handle instead.",
      parameters: Type.Object({
        projectName: Type.Optional(Type.String()),
        prompt: Type.String(),
    ...
    api.registerTool({
      ...
      description:
        "Turn screenshot/browser/mobile observations into a visual QA report that checks expected elements, layout risks, mobile risks, console errors, network errors, and next steps.",
      parameters: Type.Object({
        projectName: Type.Optional(Type.String()),
    ...
    api.registerTool({
      ...
      description:
        "Create a full ClawKit path from idea or existing Lovable app to verified preview, GitHub handoff, hardening, publish, and client handoff.",
      parameters: Type.Object({
        projectName: Type.Optional(Type.String()),
